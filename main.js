...
let currentProduct = null; let currentColor = null; let currentStep = 1;

function updateNavUI() {
    const l = document.getElementById('landing-screen'); const hN = document.getElementById('nav-home'); const sN = document.getElementById('nav-shop');
    if (!l || !hN || !sN) return;
    ...
    hN.className = isL ? active : inactive; sN.className = isL ? inactive : active;
}
...
function updateStepper() {
  for (let i = 1; i <= 4; i++) {
    ...
    if (i < currentStep) { c.innerHTML = '<span class="material-symbols-outlined text-[16px]">check</span>'; c.className = 'w-10 h-10 rounded-full flex items-center justify-center bg-secondary text-white shadow-sm'; if(label) label.className = "text-[10px] font-bold text-secondary tracking-tighter opacity-80"; } 
    else if (i === currentStep) { c.innerText = i; c.className = 'w-10 h-10 rounded-full flex items-center justify-center bg-primary text-white scale-110 shadow-xl'; if(label) label.className = "text-[10px] font-bold text-primary tracking-tighter opacity-100"; } 
    else { c.innerText = i; c.className = 'w-10 h-10 rounded-full flex items-center justify-center bg-surface-container-high text-on-surface-variant/30 text-sm'; if(label) label.className = "text-[10px] font-bold opacity-30 tracking-tighter"; }
  }
}
...
function spawnPetals(selectedTheme) {
    ...
    for (let i = 0; i < count; i++) {
        const p = document.createElement('div'); p.className = "fixed pointer-events-none z-[2000] rotate-45";
        p.style.width = `${Math.random() * 8 + 12}px`; p.style.height = `${Math.random() * 5 + 8}px`; p.style.borderRadius = "100% 15% 100% 15%";
        ...
        p.style.boxShadow = `0 0 10px rgba(255,255,255,0.7), 0 0 2px rgba(0,0,0,0.2)`; p.style.border = `1px solid rgba(255,255,255,0.2)`;
        document.body.appendChild(p);
        const delay = i * 60; const dx = (toR.left + toR.width/2) - (fromR.left + fromR.width/2); const dy = (toR.top + toR.height/2) - (fromR.top + fromR.height/2);
        const anim = p.animate([
            { transform: 'translate(0, 0) rotate(0deg) scale(0)', opacity: 0 }, { transform: `translate(${(Math.random()-0.5)*120}px, -80px) rotate(45deg) scale(1.5)`, opacity: 1, offset: 0.1 }, { transform: `translate(${dx / 2}px, -200px) rotate(180deg) scale(1.8)`, opacity: 1, offset: 0.4 }, { transform: `translate(${dx - 40}px, ${dy - 20}px) rotate(360deg) scale(1.5)`, opacity: 1, offset: 0.7 }, { transform: `translate(${dx + 40}px, ${dy - 40}px) rotate(540deg) scale(1.5)`, opacity: 1, offset: 0.85 }, { transform: `translate(${dx}px, ${dy - 100}px) rotate(630deg) scale(1.2)`, opacity: 1, offset: 0.92 }, { transform: `translate(${dx}px, ${dy}px) rotate(720deg) scale(0.1)`, opacity: 0, offset: 1 }
...
function renderProducts() {
    ...
    PRODUCTS.filter(c => c.purposes.includes(currentPurpose)).forEach(cat => {
        const div = document.createElement('div'); div.className = "mb-16"; div.innerHTML = `<h4 class="text-2xl font-headline font-bold text-primary mb-8 border-b-2 border-primary/10 pb-4 text-center md:text-left">${cat.title}</h4><div class="grid grid-cols-1 md:grid-cols-3 gap-10" id="cat-${cat.id}"></div>`;
...
function updateCartUI() {
    const list = document.getElementById('cart-items-list'); const badge = document.getElementById('cart-badge-count'); const disp = document.getElementById('cart-total-display');
    ...
    if(badge){ badge.innerText = cart.length; badge.classList.toggle('hidden', cart.length === 0); } if(disp) disp.innerText = `¥${t.toLocaleString()}`;
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const cards = document.querySelectorAll('.purpose-card'); cards.forEach(card => card.onclick = () => { currentPurpose = card.dataset.purpose; showStep(2); });
    const addBtn = document.getElementById('add-to-cart-btn'); if(addBtn) addBtn.onclick = () => {
        ...
        spawnPetals(currentColor); const acc = document.getElementById('add-to-cart-container'); if(acc) acc.classList.add('opacity-0');
        updateCartUI();
    };
