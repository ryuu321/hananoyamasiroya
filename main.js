...
function updateStepper() {
  for (let i = 1; i <= 4; i++) {
    ...
    if (i < currentStep) { c.innerHTML = '<span class="material-symbols-outlined text-sm">check</span>'; c.className = 'w-12 h-12 rounded-full flex-shrink-0 flex items-center justify-center bg-secondary text-white shadow-md'; } 
    else if (i === currentStep) { c.innerText = i; c.className = 'w-12 h-12 rounded-full flex-shrink-0 flex items-center justify-center bg-primary text-white scale-110 shadow-2xl transition-all duration-500'; } 
    else { c.innerText = i; c.className = 'w-12 h-12 rounded-full flex-shrink-0 flex items-center justify-center bg-surface-container text-primary font-bold opacity-30'; }
  }
}
